...
def test_get_connection_raises_custom_error_on_duckdb_error(mock_connect):
    ...
    with pytest.raises(
        DatabaseConnectionError, match="Failed to connect to database"
    ):
        db.get_connection()


@patch("flashcore.db.database.duckdb.connect")
def test_initialize_schema_raises_custom_error_on_duckdb_error(
    mock_duckdb_connect,
):
    ...
    with pytest.raises(
        SchemaInitializationError, match="Failed to initialize schema"
    ):
...
def test_initialize_schema_handles_rollback_error(
    mock_duckdb_connect, mock_logger_error
):
    """
    ...
    with pytest.raises(
        SchemaInitializationError,
        match="Failed to initialize schema: Initial schema error",
    ):
        db.initialize_schema()
...
def test_upsert_cards_batch_handles_db_error(mock_duckdb_connect):
    ...
    mock_connection = MagicMock()
    mock_connection.cursor.return_value.__enter__.return_value = mock_cursor
    type(mock_connection).closed = PropertyMock(
        return_value=False
    ...
    with pytest.raises(
        CardOperationError, match=r"Batch card upsert failed:.*Upsert failed"
    ):
...
def test_get_card_by_uuid_handles_validation_error(
    ...
    marshalling_error = MarshallingError(
        "Marshalling failed", original_exception=validation_error
    )
    ...
    assert (
        f"Failed to parse card with UUID {card_uuid_to_fetch} from database"
...
def test_get_all_cards_handles_db_error(mock_duckdb_connect):
    ...
    with pytest.raises(
        CardOperationError, match="Failed to get all cards: DB query failed"
    ):
        db.get_all_cards()
...
def test_get_due_card_count_handles_db_error(mock_duckdb_connect):
    ...
    # 2. Execute and assert
    with pytest.raises(
        CardOperationError, match="Failed to count due cards: DB count failed"
    ):
        db.get_due_card_count(
            deck_name="any_deck", on_date=datetime.now(timezone.utc).date()
        )
...
def test_delete_cards_by_uuids_batch_handles_db_error(mock_duckdb_connect):
    ...
    # 2. Execute and assert
    with pytest.raises(
        CardOperationError, match="Batch card delete failed: Delete failed"
    ):
...
@patch("flashcore.db.database.duckdb.connect")
def test_upsert_cards_batch_handles_rollback_error(
    mock_duckdb_connect, caplog
):
...
def test_get_card_by_uuid_handles_db_error(mock_duckdb_connect):
    ...
    with pytest.raises(
        CardOperationError, match="Failed to fetch card by UUID"
    ):
...
def test_get_due_cards_handles_db_error(mock_duckdb_connect):
    ...
    # 1. Setup mock to raise an error on execute
    mock_connection = MagicMock()
    mock_connection.execute.side_effect = duckdb.Error(
        "DB query failed for due cards"
    )
    mock_duckdb_connect.return_value = mock_connection
...
def test_add_review_and_update_card_handles_db_error(mock_duckdb_connect):
    ...
    mock_cursor = MagicMock()
    mock_cursor.execute.side_effect = duckdb.Error(
        "DB query failed during transaction"
    )
    ...
    with pytest.raises(
        DatabaseError, match="Failed to add review and update card"
    ):
...
def test_get_reviews_for_card_handles_db_error(mock_duckdb_connect):
    ...
    # 2. Execute and assert that the error is caught and re-raised correctly
    with pytest.raises(
        ReviewOperationError, match="Failed to get reviews for card"
    ):
        db.get_reviews_for_card(sample_uuid)


@patch("flashcore.db.database.duckdb.connect")
def test_add_review_and_update_card_handles_missing_return_id(
    mock_duckdb_connect,
):
    ...
    mock_connection = MagicMock()
    mock_connection.cursor.return_value.__enter__.return_value = mock_cursor
    mock_connection.closed = (
        False  # Simulate an open connection so rollback is attempted
    )
    mock_duckdb_connect.return_value = mock_connection
...
def test_add_review_and_update_card_read_only_mode_raises_error(tmp_path):
    ...
    with pytest.raises(
        DatabaseConnectionError, match="Cannot add review in read-only mode"
    ):
        db.add_review_and_update_card(review, new_card_state)


@patch(
    "flashcore.db.database.FlashcardDatabase._insert_review_and_get_id",
    side_effect=ValueError("Internal processing error"),
)
def test_add_review_and_update_card_handles_generic_exception(
    mock_insert_review,
):
    """
...
@patch(
    "flashcore.db.database.FlashcardDatabase._insert_review_and_get_id",
    side_effect=CardOperationError("Underlying DB issue"),
)
def test_add_review_and_update_card_reraises_database_error(
    mock_insert_review,
):
    """
...
def test_get_all_cards_handles_validation_error(
    ...
    assert isinstance(
        excinfo.value.original_exception.original_exception, ValidationError
    )
...
def test_get_card_with_invalid_tag_data_raises_error():
    ...
    # Verify the exception chain is CardOperationError -> MarshallingError -> ValidationError
    assert f"Failed to parse card with UUID {card_uuid} from database" in str(
        excinfo.value
    )
    assert isinstance(excinfo.value.original_exception, MarshallingError)
    assert isinstance(
...
@patch("flashcore.db.database.duckdb.connect")
def test_add_review_and_update_card_handles_rollback_error(
    mock_duckdb_connect, caplog
):
    ...
    mock_connection = MagicMock()
    mock_connection.cursor.return_value.__enter__.return_value = mock_cursor
    mock_connection.rollback.side_effect = duckdb.Error(
        "Rollback failed miserably"
    )
    ...
    with pytest.raises(
        ReviewOperationError,
        match="Failed to add review and update card: Review insert failed",
    ):
        db.add_review_and_update_card(sample_review, CardState.Review)

    # 3. Verify the fatal rollback error was logged
    assert (
        "Failed to rollback transaction: Rollback failed miserably"
        in caplog.text
    )
