...
class TestSchemaManagerCoverage:
    ...
    def test_handle_read_only_initialization_force_recreate_error(self, tmp_path):
        ...
        with pytest.raises(DatabaseConnectionError, match="Cannot force recreate tables in read-only mode"):
            manager.initialize_schema(force_recreate_tables=True)

    def test_safety_check_with_data(self, initialized_db_manager, sample_card1):
        """Test safety check prevents dropping tables with data."""
        db = initialized_db_manager
        db.upsert_cards_batch([sample_card1])
        
        # Try to force recreate with data present
...
class TestDatabaseCoverage:
    ...
    def test_upsert_error_handling_with_closed_connection(self, initialized_db_manager, sample_card1):
        ...
        with patch.object(db, 'get_connection') as mock_conn:
            mock_conn.return_value.cursor.return_value.__enter__.return_value.executemany.side_effect = duckdb.Error("Test error")
