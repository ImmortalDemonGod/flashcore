...
def validate_card_uuid(
    ...
    # Check uuid field first (preferred), then fall back to id field for backward compatibility
    ...
    try:
        ...
        return YAMLProcessingError(
            ...
            message=f"Invalid UUID format for '{field_name}': '{uuid_value}'.",
        )
...
def _handle_skipped_media_validation(media_paths: List[str]) -> List[Path]:
    """Converts media paths to Path objects without validation, ensuring it's a list."""
...
def _validate_media_path_list(
    media_paths: List[str], context: _CardProcessingContext
...
def validate_media_paths(
    ...
    if not isinstance(media_paths, list):
        return YAMLProcessingError(
            ...
            f"The 'media' field must be a list of strings, but got {type(media_paths).__name__}.",
            card_index=context.card_index,
            card_question_snippet=context.card_q_preview,
            field_name="media",
        )
...
def validate_single_media_path(
        ...
        try:
            ...
            if full_media_path.is_dir():
                return YAMLProcessingError(
                    ...
                    card_question_snippet=context.card_q_preview,
                    field_name="media",
                    message=f"Media path is a directory, not a file: '{media_path}'.",
                )
            # On success, return the fully resolved and validated path
    ...
    # If validation is skipped, return the original path as a Path object
...
def validate_raw_card_structure(
    ...
    # Work on a copy to prevent side effects from modifying the original dict by reference.
