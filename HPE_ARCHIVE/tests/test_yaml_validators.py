...
from cultivation.scripts.flashcore.yaml_processing.yaml_validators import (
    validate_card_uuid,
    check_for_secrets,
...
from cultivation.scripts.flashcore.yaml_processing.yaml_models import (
    _RawYAMLCardEntry,
...
def test_check_for_secrets_found_in_question(mock_context):
    ...
    secret_string = "api_key: 'this_is_a_super_long_secret_key_that_is_over_20_chars'"
...
def test_check_for_secrets_found_in_answer(mock_context):
    ...
    secret_string = "here is a github token ghp_abcdefghijklmnopqrstuvwxyz1234567890abcd"
...
def test_validate_directories_assets_do_not_exist(tmp_path):
    ...
    source_dir.mkdir()
    result = validate_directories(source_dir, Path("/nonexistent/assets"), False)
    assert isinstance(result, YAMLProcessingError)
...
def test_extract_cards_list_missing():
    ...
    with pytest.raises(YAMLProcessingError, match="Missing or invalid 'cards' list"):
        extract_cards_list({}, Path("test.yaml"))

def test_extract_cards_list_empty():
    """Tests error when 'cards' list is empty."""
    with pytest.raises(YAMLProcessingError, match="No cards found in 'cards' list"):
...
def test_validate_raw_card_structure_invalid(tmp_path):
    ...
    result = validate_raw_card_structure(invalid_card_dict, 0, tmp_path / "test.yaml")
    assert isinstance(result, YAMLProcessingError)
...
def test_validate_raw_card_structure_not_a_dict(tmp_path):
    ...
    result = validate_raw_card_structure(invalid_card, 0, tmp_path / "test.yaml")
    assert isinstance(result, YAMLProcessingError)
...
def test_validate_single_media_path_generic_exception(mock_context, mocker):
    ...
    mocker.patch.object(Path, "resolve", side_effect=Exception("Unexpected error"))
...
class TestDeckMetadataValidation:
    def test_extract_deck_name_success(self):
        assert extract_deck_name({"deck": " My Deck "}, Path("test.yaml")) == "My Deck"
    ...
    def test_extract_deck_name_not_a_string(self):
        with pytest.raises(YAMLProcessingError, match="'deck' field must be a string"):
            extract_deck_name({"deck": 123}, Path("test.yaml"))

    def test_extract_deck_name_empty_string(self):
        with pytest.raises(YAMLProcessingError, match="'deck' field cannot be empty"):
            extract_deck_name({"deck": "   "}, Path("test.yaml"))

    def test_extract_deck_tags_success(self):
        tags = extract_deck_tags({"tags": [" Tag1 ", "tag2"]}, Path("test.yaml"))
    ...
    def test_extract_deck_tags_not_a_list(self):
        with pytest.raises(YAMLProcessingError, match="'tags' field must be a list"):
...
def test_check_for_secrets_no_secret(mock_context):
    ...
    result = check_for_secrets("Normal question", "Normal answer", mock_context)
    assert result is None
...
def test_validate_directories_assets_skipped(tmp_path):
    ...
    source_dir = tmp_path / "source"
    source_dir.mkdir()
    result = validate_directories(source_dir, Path("/nonexistent/assets"), True)
    assert result is None
...
class TestValidateDeckAndExtractMetadata:
    def test_success(self):
        ...
        deck, tags, cards = validate_deck_and_extract_metadata(raw_content, Path("f.yaml"))
        assert deck == "My Deck"
        assert tags == {"d1"}
        assert len(cards) == 1
...
class TestValidationHappyPaths:
    ...
    def test_run_card_validation_pipeline_success(self, mock_context):
        ...
        # Act
        result = run_card_validation_pipeline(raw_card, mock_context, deck_tags)
