...
from cultivation.scripts.flashcore.yaml_processing.yaml_processor import (
    YAMLProcessorConfig,
    load_and_process_flashcard_yamls,
)
from cultivation.scripts.flashcore.yaml_processing.yaml_models import YAMLProcessingError
...
class TestLoadAndProcessFlashcardYamls:
    ...
    def test_error_aggregation_fail_fast_false(self, tmp_path: Path, assets_dir: Path):
        ...
        assert any("Field required" in str(e) for e in errors if e.file_path.name == "card_no_q.yaml")
...
'''
        ...
        with pytest.raises(YAMLProcessingError, match="Validation error in field 'cards.1.q': Field required"):
            config = YAMLProcessorConfig(source_directory=source_dir, assets_root_directory=assets_dir, fail_fast=True)
            load_and_process_flashcard_yamls(config)
...
'''
        ...
        # The first instance of "Shared Question?" is kept, plus "Unique Question?"
        assert len(cards) == 2
        assert len(errors) == 1
        error = errors[0]
        assert "Cross-file duplicate question front" in error.message
        assert "deckA.yaml" in error.message # Should reference the first file
        assert error.file_path.name == "deckB.yaml" # Error is reported for the second file
        assert error.card_question_snippet == "Shared Question?"
...
'''
    ...
    def test_intra_file_duplicate_question(self, tmp_path: Path, assets_dir: Path):
        ...
        # The first "Duplicate Question?" and "Unique Question?" are kept.
        assert len(cards) == 2
        assert len(errors) == 1
        error = errors[0]
        assert "Duplicate question front within this YAML file" in error.message
        assert error.file_path.name == "intradup.yaml"
        assert error.card_index == 2 # The third card (index 2) is the duplicate
        assert error.card_question_snippet == "Duplicate Question?"
    ...
    def test_invalid_schema_no_deck(self, tmp_path: Path, assets_dir: Path):
        ...
        assert not cards
        assert len(errors) == 1
        assert "Validation error in field 'deck': Field required" in errors[0].message
...
"""
    ...
    def test_card_entry_not_a_dictionary(self, tmp_path: Path, assets_dir: Path):
...
"""
        ...
        assert not cards
        assert len(errors) == 1
        assert "Input should be a valid dictionary or instance" in errors[0].message
    ...
    def test_io_error_on_read(self, tmp_path: Path, assets_dir: Path, mocker):
        ...
        mocker.patch.object(Path, "read_text", side_effect=IOError("Disk on fire"))
    ...
    def test_file_not_found_error_on_read(self, tmp_path: Path, assets_dir: Path, mocker):
        source_dir = tmp_path / "src_fnf_error"
        source_dir.mkdir()
        # The file exists when found, but we mock the read to fail as if it was deleted.
        create_yaml_file(source_dir, "deck.yaml", VALID_YAML_MINIMAL_CONTENT)

        mocker.patch.object(Path, "read_text", side_effect=FileNotFoundError("File disappeared"))
    ...
    def test_unexpected_error_during_file_processing(self, tmp_path: Path, assets_dir: Path, mocker):
        ...
        mocker.patch(
            "cultivation.scripts.flashcore.yaml_processing.yaml_processor.YAMLProcessor.process_file",
        ...
        assert not cards
        assert len(errors) == 1
        assert "An unexpected error occurred while processing" in errors[0].message
        assert "Something went very wrong" in errors[0].message

    def test_unexpected_critical_error_in_run(self, tmp_path: Path, assets_dir: Path, mocker):
        source_dir = tmp_path / "src_critical_run_error"
        source_dir.mkdir()

        # Mock a method early in the `run` process to simulate a critical failure
        mocker.patch("pathlib.Path.rglob", side_effect=Exception("Critical failure"))
